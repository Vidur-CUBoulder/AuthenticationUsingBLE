...
static uint8_t print_buffer[34];
uint8_t * ptr_print;
	...
	if(Flags & LEUART_IF_RXDATAV)
	{
		...
		if(rx_data_count == 16) {
			...
			case set_new_pin:
				...
				printf("\f");
				printf("New Pin Set! Connect to BLE\n");
				...
				GPIO_PinOutClear(gpioPortF,5);				//Set Led 1 low
			...
			case check_pin:
						...
						)
				{
					...
					memset(ptr_pinvalue,0,6);				//resest old pin to 0
					gecko_cmd_gatt_server_write_attribute_value(gattdb_pin_value,0,6,ptr_pinvalue);


					gecko_cmd_endpoint_close(0xFF);
			    	gecko_cmd_le_gap_set_adv_parameters(800,800,0);
			    	/* Start general advertising and enable connections. */
...
//    	gecko_cmd_le_gap_set_mode(le_gap_general_discoverable, le_gap_undirected_connectable);
    	...
    	gecko_cmd_hardware_set_soft_timer(TIMEOUT,0,0);	//Set 2 minutes timeout
    	gecko_cmd_system_set_tx_power(-300);			//Reduce power level for short range working

    	Setup_LEUART0();								//Enable all the required peripherals
    	Setup_LEUART_Rx();
    	Setup_CRYPTO();
      ...
      case gecko_evt_hardware_soft_timer_id:
    	  GPIO_PinOutSet(gpioPortF,5);					//Set LED 1 high
